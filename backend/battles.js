 ...
 * {
 *   mode: 'pvp' | 'bot',
 *   players: string[],           // hier 1 Spieler (Demo) + servergesteuerter Gegner
 ...
 *   locks: {
 *     player1?: { type: 'move'|'switch', index: number }, // index = moveIndex oder switchIndex
 *     player2?: { ... }
 *   }
 * }
 *
...
async function pickRealMoves(pokemonData) {
  const take = async (list) => Promise.all(list.map(async (m) => {
    try {
      ...
      return { name: mv.name, power: mv.power, type: mv.type?.name, category: mv.damage_class?.name };
    } catch { return null; }
  }));
  ...
  for (const [, arr] of byType) {
    selected.push(arr[0]); if (selected.length>=4) break;
  }
  let i=0;
  while (selected.length<4 && i<usable.length) { if (!selected.includes(usable[i])) selected.push(usable[i]); i++; }
...
// ---------------------- Turn Engine ----------------------
function ensureSelectPhase(state){
  state.phase = 'select';
  state.locks = {};
}
function orderActions(state){
  // Priorität: Switch vor Move; bei gleichem Typ vergleichen wir Speed (höher zuerst).
  const entries = [
    { side: 'player1', lock: state.locks.player1 },
    { side: 'player2', lock: state.locks.player2 }
  ];
  return entries.sort((a,b)=>{
    const prioA = a.lock?.type === 'switch' ? 2 : 1;
    const prioB = b.lock?.type === 'switch' ? 2 : 1;
    if (prioA !== prioB) return prioB - prioA; // Switch zuerst
    // gleich: Speed
    const spA = state.teams[a.side][state.active[a.side]].stats.speed;
    const spB = state.teams[b.side][state.active[b.side]].stats.speed;
    if (spA !== spB) return spB - spA; // schneller zuerst
    // tie: zufällig
    return Math.random()<0.5 ? -1 : 1;
  });
}

async function resolveTurn(io, room){
  const state = rooms.get(room);
  if (!state || state.over) return;
  state.phase = 'resolve';
  io.to(room).emit('turn-state', { phase: 'resolve', locks: state.locks });

  const sequence = orderActions(state);
  for (const act of sequence){
    ...
    // Falls der Ausführende bereits kampfunfähig ist, Aktion verfällt
    const atkMon = state.teams[side][state.active[side]];
    if (atkMon.currentHp <= 0) continue;
    ...
    if (lock.type === 'move'){
      const moveIndex = lock.index;
      const attacker = state.teams[side][state.active[side]];
      const defender = state.teams[opp][state.active[opp]];
      if (!attacker?.moves?.[moveIndex] || defender.currentHp<=0) continue;
  ...
  ensureSelectPhase(state);
  io.to(room).emit('turn-state', { phase: 'select', locks: state.locks });
...
export async function startPvpQuickMatch(io, socket, gens=1){
  ...
  socket.join(room);
  io.to(room).emit('battle-start', { room, teams: state.teams, active: state.active, phase: state.phase, locks: state.locks });
}
...
/** payload:
 * { room, side?: 'player1'|'player2', type: 'move'|'switch', index: number }
 *  - move: index = moveIndex (0..3)
 *  - switch: index = teamIndex
 */
export async function clientLockAction(io, socket, payload){
  ...
  if (state.phase !== 'select'){
    io.to(room).emit('error-message', 'Aktionen können nur in der Auswahlphase gelockt werden.');
    return;
  }
  ...
  if (type === 'switch'){
    ...
    state.locks[side] = { type:'switch', index };
  } else if (type === 'move'){
    ...
    state.locks[side] = { type:'move', index };
  } else {
    return io.to(room).emit('error-message','Unbekannter Aktionstyp.');
  }

  // Bot lockt gleichzeitig
  if (state.mode==='bot' && side==='player1'){
    const botSide = 'player2';
    const atk = state.teams[botSide][state.active[botSide]];
    const def = state.teams['player1'][state.active['player1']];
    // wähle best move nach (power * effectiveness); 20% chance auf defensiven Switch wenn HP < 30%
    let botAction;
    if (atk.currentHp/atk.stats.hp < 0.3 && Math.random()<0.2){
      // Defensive Switch wenn möglich
      let switched = null;
      for (let i=0;i<state.teams[botSide].length;i++){
        if (i!==state.active[botSide] && state.teams[botSide][i].currentHp>0){ switched = i; break; }
      }
      if (switched!==null) botAction = { type:'switch', index: switched };
    }
    if (!botAction){
      let bestIdx = 0; let bestScore = -Infinity;
      atk.moves.forEach((m, idx) => {
        const eff = typeMultiplier(m.type, def.types);
        const score = (m.power || 40) * eff;
        if (score > bestScore){ bestScore=score; bestIdx=idx; }
      });
      botAction = { type:'move', index: bestIdx };
    }
    state.locks[botSide] = botAction;
  }

  // Status an Clients
  io.to(room).emit('turn-state', { phase: 'select', locks: state.locks });

  // Wenn beide Seiten gelockt → Resolve
  if (state.locks.player1 && state.locks.player2){
    await resolveTurn(io, room);
  }
}
